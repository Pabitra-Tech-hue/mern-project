...
exports.create = (0, catchAsync_utils_1.catchAsync)(async (req, res) => {
    ...
    //* upload cover_image
    const { path, public_id } = await (0, cloudinary_utils_1.uploadFileToCloudinary)(cover_image[0], folder);
    product.cover_image = {
        path,
        public_id,
    };
    ...
    if (images && images.length > 0) {
        const promises = images.map((file) => (0, cloudinary_utils_1.uploadFileToCloudinary)(file, folder));
...
exports.remove = (0, catchAsync_utils_1.catchAsync)(async (req, res) => {
    ...
    if (!product)
        throw new appError_utils_1.default(`product not found`, 404);
    //* delete cover image
    (0, cloudinary_utils_1.deleteFileFormCloudinary)(product.cover_image.public_id);
    //*delete images
    if (product.images && product.images.length > 0) {
        Promise.allSettled(product.images.map((img) => (0, cloudinary_utils_1.deleteFileFormCloudinary)(img.public_id)));
    }
    //* delete product
...
exports.update = (0, catchAsync_utils_1.catchAsync)(async (req, res) => {
    ...
    if (deleted_images &&
        ...
        Promise.allSettled(deleted_images.map((public_id) => (0, cloudinary_utils_1.deleteFileFormCloudinary)(public_id)));
        product.images = product.images.filter((img) => !deleted_images.includes(img.public_id.toString()));
    }
    //* if new images
    if (images && images.length > 0) {
        // [{status:'',value:{path,public_id}}]
        const files = await Promise.allSettled(images.map((file) => (0, cloudinary_utils_1.uploadFileToCloudinary)(file, folder)));
